...
function App() {
  ...
  const handlePrintRoute = () => {
    ...
    const timePeriod = timeOfDay.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    const dayLabel   = dayType.charAt(0).toUpperCase() + dayType.slice(1);

    const routeBlocks = result.routes.map(route => {
      const rows = route.stops.map((stop, i) => {
        const isStart  = i === 0;
        const isReturn = i === route.stops.length - 1;
        const label    = isStart ? 'Depot (Start)' : isReturn ? 'Depot (Return)' : `Stop ${i}`;
        const etaText  = stop.eta_minutes === 0 ? '—' : formatTime(stop.eta_minutes);
        const rowBg    = i % 2 === 0 ? '#f9fafb' : '#ffffff';
        return `
          <tr style="background:${rowBg}">
            <td style="padding:10px 14px;font-weight:600;color:#374151;white-space:nowrap">${label}</td>
            <td style="padding:10px 14px;font-size:1rem;color:#111827">${stop.name}</td>
            <td style="padding:10px 14px;text-align:right;white-space:nowrap;color:#374151">
              ${etaText}${stop.eta_minutes > 0 ? '<span style="font-size:0.78rem;color:#9ca3af;margin-left:4px">±5 min</span>' : ''}
            </td>
          </tr>`;
      }).join('');
      ...
      return `
        <div style="margin-bottom:28px;page-break-inside:avoid">
          <div style="background:#1e3a5f;color:white;padding:8px 14px;border-radius:6px 6px 0 0;font-weight:700;font-size:0.9rem;letter-spacing:0.5px">
            VEHICLE ${route.vehicle_id}
            <span style="font-weight:400;font-size:0.8rem;margin-left:12px;opacity:0.8">
              ${route.distance_km} km &nbsp;|&nbsp; ${formatTime(route.total_time_minutes)} total
            </span>
          </div>
          <table style="width:100%;border-collapse:collapse;border:1px solid #e5e7eb;border-top:none">
            <thead>
              <tr style="background:#f3f4f6">
                <th style="padding:8px 14px;text-align:left;font-size:0.78rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;width:140px">Sequence</th>
                <th style="padding:8px 14px;text-align:left;font-size:0.78rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px">Location</th>
                <th style="padding:8px 14px;text-align:right;font-size:0.78rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;width:120px">ETA from start</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
...
<head>
  <meta charset="UTF-8"/>
  <title>LogiOptic Route Sheet</title>
  <style>
...
<body>
  <!-- Header -->
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid #1e3a5f">
    <div>
      <div style="font-size:1.7rem;font-weight:800;letter-spacing:2px;color:#1e3a5f">LogiOptic</div>
      <div style="font-size:0.82rem;color:#6b7280;margin-top:2px">AI-Powered Route Optimization</div>
    </div>
    <div style="text-align:right">
      <div style="font-size:0.85rem;font-weight:700;color:#374151">DRIVER ROUTE SHEET</div>
      <div style="font-size:0.78rem;color:#9ca3af;margin-top:3px">Printed: ${printDate}</div>
    </div>
  </div>

  <!-- Route summary bar -->
  <div style="background:#f3f4f6;border:1px solid #e5e7eb;border-radius:7px;padding:10px 16px;margin-bottom:24px;font-size:0.88rem;color:#374151;display:flex;flex-wrap:wrap;gap:6px 0">
    <span><strong>Time period:</strong> ${timePeriod} &nbsp;&nbsp;</span>
    <span><strong>Day:</strong> ${dayLabel} &nbsp;&nbsp;</span>
    <span><strong>Total distance:</strong> ${result.total_distance_km} km &nbsp;&nbsp;</span>
    <span><strong>Total time:</strong> ${formatTime(result.total_time_minutes)} &nbsp;&nbsp;</span>
    <span><strong>Stops:</strong> ${result.num_stops}</span>
  </div>
  ...
  <!-- Footer note -->
  <div style="margin-top:24px;padding-top:12px;border-top:1px solid #e5e7eb;font-size:0.75rem;color:#9ca3af">
    ETA values are AI-predicted travel times from the depot. Actual times may vary with live traffic conditions.
    &nbsp; Generated by LogiOptic &mdash; XGBoost + OR-Tools route optimization.
  </div>
        ...
        <div className="panel results-panel">
          ...
          {result && (
            <div className="results-section">
              ...
              <button className="btn-export" onClick={handlePrintRoute}>
                🖨 Print Route Sheet
              </button>
                    ...
                    <tbody>
                      {route.stops.map((stop, i) => (
                        <tr key={i}>
                          ...
                          <td>{stop.name}</td>
                          <td>
                            {formatTime(stop.eta_minutes)}
                            {stop.eta_minutes > 0 && (
                              <span className="eta-confidence"> ±5 min</span>
                            )}
                          </td>
                        </tr>
                      ))}
                  ...
                  {['morning_peak', 'midday', 'evening_peak', 'night'].map(period => {
                    ...
                    return (
                      <tr key={period} className={period === timeOfDay ? 'active-row' : ''}>
                        ...
                        <td>{r.error ? '—' : formatTime(r.total_time_minutes)}</td>
